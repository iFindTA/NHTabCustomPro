...
{    
    ...
    self.badgeOverlayColor = kDefaultBadgeBackgroundColor;
    self.badgeTextColor = kDefaultBadgeTextColor;
    self.badgeTextShadowColor = kDefaultBadgeShadowColor;
    self.badgeTextFont = [UIFont fontWithName:@"Helvetica-Bold" size:11.0];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw){
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextSetStrokeColorWithColor(ctx, kDefaultBadgeBackgroundColor.CGColor);
        ...
        CGContextSaveGState(ctx);
        {
            ...
            textFrame.origin.y = rectToDraw.origin.y + ceilf((rectToDraw.size.height - textSize.height) /2);
    ...
    {
        //2014.2.11 nanhu modefied
        //没有要显示的内容 就默认显示一个红原点
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        
        CGRect rectToDraw = CGRectInset(rect, kMarginToDrawInside, kMarginToDrawInside*2);
        UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(kBadgeCornerRadius, kBadgeCornerRadius)];
